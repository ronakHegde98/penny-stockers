...
def get_sheet(spreadsheet, sheet_name):
    ...
    try:
        ...
        logging.criticial(
            f"The sheet {sheet_name} was not found in {spreadsheet.title}"
        )
...
def get_column_values(sheet, col_index: int, starting_row_index: int = 1) -> List[str]:
    col_values = sheet.col_values(col_index)[starting_row_index:]
    return [val.strip() for val in col_values]
